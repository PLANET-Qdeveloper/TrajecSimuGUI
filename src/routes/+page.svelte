...
<div class="flex flex-col h-screen overflow-hidden bg-white text-black">
    ...
    <div class="flex-1 overflow-hidden relative">
            ...
            <div class="flex flex-col overflow-hidden" style="flex: 1">
                ...
                >
                    <label class="flex items-center gap-1 cursor-pointer select-none">
                        <input class="accent-primary" type="checkbox" bind:checked={showTrajectoryMarker}/>
                        マーカー
                    </label>
                    <label class="flex items-center gap-1 cursor-pointer select-none">
                        <input class="accent-primary" type="checkbox" bind:checked={showBallisticCourse}/>
                        <span class="">弾道軌跡</span>
                    </label>
                    <label class="flex items-center gap-1 cursor-pointer select-none">
                        <input class="accent-primary" type="checkbox" bind:checked={showParachuteCourse}/>
                        <span class="">落下傘軌跡</span>
                    </label>
                    <label class="flex items-center gap-1 cursor-pointer select-none">
                        <input class="accent-primary" type="checkbox" bind:checked={showBallisticLandingRange}/>
                        <span class="">弾道範囲</span>
                    </label>
                    <label class="flex items-center gap-1 cursor-pointer select-none">
                        <input class="accent-primary" type="checkbox" bind:checked={showParachuteLandingRange}/>
                        <span class="">落下傘範囲</span>
                    </label>
                    <label class="flex items-center gap-1 cursor-pointer select-none">
                        <input class="accent-primary" type="checkbox" bind:checked={showImportedKmlOverlay}/>
        ...
        <!-- result タブ -->
        <div class="absolute inset-0 flex overflow-hidden" class:hidden={activeTab !== "map"}>
            ...
            <div class="flex-[2] min-w-0 flex flex-col overflow-hidden border-l">
                ...
                <div class="flex-1 min-h-0">
                    <ChartMap
                            ...
                            value={result ? (result.trajectory_ballistic as unknown as Record<string, number[]>)[selectedChartMapValue] : []}
                            overlayKmlString={overlayKmlString}
                            showImportedKmlOverlay={showImportedKmlOverlay}
                    />
                </div>
            </div>
        </div>
