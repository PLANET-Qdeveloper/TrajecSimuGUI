...
<details open>
  ...
  <div class="mt-1 grid grid-cols-3 gap-x-2 gap-y-1">
    ...
    <label class="col-span-2 flex flex-col gap-0.5">
      ...
      <Input type="number" step="0.01" min="0" bind:value={body.dry_mass_with_fuel_section} />
    </label>
    ...
    <!-- 慣性テンソル -->
    <div class="col-span-3 flex flex-col gap-0.5">
      <span class="text-[10px] text-gray-500">慣性テンソル (kg·m²) — Ixx, Iyy, Izz, Ixy, Ixz, Iyz</span>
      <div class="grid grid-cols-3 gap-1">
        {#each ['Ixx', 'Iyy', 'Izz', 'Ixy', 'Ixz', 'Iyz'] as label, i}
          <label class="flex flex-col gap-0.5">
            ...
            <Input
              ...
              oninput={(e) => {
                const v = [...body.inertia] as [number, number, number, number, number, number];
                v[i] = parseFloat((e.target as HTMLInputElement).value) || 0;
                body.inertia = v;
              }}
            />
