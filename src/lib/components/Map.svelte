<script lang="ts">
    ...
    function updateTrajectory(kmlStr: string | null) {
        ...
        const ballistic_paths = geojson.features
            ...
            .map(f => ({
                ...
                path: (f.geometry as { coordinates: number[][] }).coordinates as [number, number, number][],
            }));
        ...
        const points = geojson.features
            ...
            .map(f => ({
                position: (f.geometry as { coordinates: number[] }).coordinates as [number, number, number],
        ...
        for (const f of geojson.features) {
            ...
            if (geom.type === 'LineString') {
                for (const c of (geom as { coordinates: number[][] }).coordinates) coords.push([c[0], c[1]]);
    ...
    function updateOverlay(kmlStr: string | null) {
        ...
        const source = map.getSource('kml-overlay') as maplibregl.GeoJSONSource | undefined;
        if (!source) return;
        if (!kmlStr) {
    ...
    onMount(() => {
        map = new maplibregl.Map({
            ...
            style: {
                ...
                layers: [
                    {id: 'background', type: 'background', paint: {'background-color': '#888888'}},
