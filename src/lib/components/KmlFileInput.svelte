...
>
  <button
    onclick={handleOpen}
    class="px-2.5 py-1 bg-sky-500 text-white rounded hover:bg-sky-600 cursor-pointer whitespace-nowrap"
  >
    KML / KMZ を開く
  </button>
  {#if filename}
    ...
    <button
      onclick={onclear}
      class="text-gray-400 hover:text-gray-800 cursor-pointer text-xs leading-none"
      >✕</button
    >
  {/if}
</div>
