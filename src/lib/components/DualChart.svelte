<script lang="ts">
    ...
    const keyOptions = Object.entries(keyLabel).map(([value, label]) => ({value, label}));
    ...
    $effect(() => {
        ...
        const traj =
            selectedPhase === "ballistic" ? trajectory_ballistic : trajectory_parachute;
        const y1 = selectedY1;
        const y2 = selectedY2;
...
<div class="flex flex-col h-full overflow-hidden">
    <!-- Top selector bar -->
    <div class="flex items-center gap-4 px-3 py-1.5 border-b bg-gray-50 shrink-0 text-xs">
        <div class="flex items-center gap-1.5">
            <span class="font-semibold text-gray-500 uppercase tracking-wider text-[10px] whitespace-nowrap"
                >フェーズ</span
            >
